...
@pytest.mark.asyncio
class TestDeleteWebsiteEndpoint:
    """Tests for DELETE /api/v1/websites/{id} endpoint."""

    async def test_delete_website_success(self, test_client: AsyncClient) -> None:
        """Test successful website deletion with soft delete."""
        # Create website
        create_response = await test_client.post(
            "/api/v1/websites",
            json={
                "name": "Website to Delete",
                "base_url": "https://example-delete.com",
                "steps": [{"name": "crawl", "type": "crawl", "method": "api"}],
            },
        )
        assert create_response.status_code == 201
        ...
        # Delete website
        delete_response = await test_client.delete(f"/api/v1/websites/{website_id}")
        assert delete_response.status_code == 200

        data = delete_response.json()
        assert data["id"] == website_id
        assert data["name"] == "Website to Delete"
        assert "deleted_at" in data
        assert data["deleted_at"] is not None
        assert "message" in data
        assert "deleted successfully" in data["message"].lower()
        assert data["cancelled_jobs"] >= 0
        assert "cancelled_job_ids" in data
        assert "config_archived_version" in data

        # Verify website is soft deleted (should return 400 when trying to delete again)
        delete_again_response = await test_client.delete(f"/api/v1/websites/{website_id}")
    ...
    async def test_delete_website_with_active_jobs(
        ...
        # Create a job for this website
        job_response = await test_client.post(
            "/api/v1/jobs/seed",
            json={
                "website_id": website_id,
                "seed_url": "https://example-jobs.com/articles",
            },
        )
        assert job_response.status_code == 201
        job_id = job_response.json()["id"]

        # Delete website (should cancel the job)
        delete_response = await test_client.delete(f"/api/v1/websites/{website_id}")
        assert delete_response.status_code == 200
    ...
    async def test_delete_website_already_deleted(self, test_client: AsyncClient) -> None:
        """Test deleting already deleted website returns 400."""
        # Create and delete website
        create_response = await test_client.post(
            "/api/v1/websites",
            json={
                "name": "Already Deleted",
                "base_url": "https://example-already-deleted.com",
                "steps": [{"name": "crawl", "type": "crawl", "method": "api"}],
            },
        )
        website_id = create_response.json()["id"]
        ...
        # First delete
        delete_response1 = await test_client.delete(f"/api/v1/websites/{website_id}")
        assert delete_response1.status_code == 200

        # Second delete (should fail)
        delete_response2 = await test_client.delete(f"/api/v1/websites/{website_id}")
        assert delete_response2.status_code == 400

        error = delete_response2.json()
        assert "already deleted" in error["detail"].lower()

    async def test_delete_website_invalid_uuid(self, test_client: AsyncClient) -> None:
        """Test deleting with invalid UUID format."""
        response = await test_client.delete("/api/v1/websites/invalid-uuid")
        assert response.status_code in [400, 422]  # Validation error

    async def test_delete_website_with_delete_data_param(self, test_client: AsyncClient) -> None:
        """Test delete endpoint with delete_data parameter (reserved for future)."""
        # Create website
        create_response = await test_client.post(
            "/api/v1/websites",
            json={
                "name": "Delete with Data",
                "base_url": "https://example-delete-data.com",
                "steps": [{"name": "crawl", "type": "crawl", "method": "api"}],
            },
        )
        website_id = create_response.json()["id"]

        # Delete with delete_data=true (currently does nothing, reserved for future)
        response = await test_client.delete(f"/api/v1/websites/{website_id}?delete_data=true")
        assert response.status_code == 200
    ...
    async def test_delete_website_no_active_jobs(self, test_client: AsyncClient) -> None:
        """Test deleting website with no active jobs."""
        # Create website
        create_response = await test_client.post(
            "/api/v1/websites",
            json={
                "name": "No Jobs Website",
                "base_url": "https://example-no-jobs.com",
                "steps": [{"name": "crawl", "type": "crawl", "method": "api"}],
            },
        )
        website_id = create_response.json()["id"]

        # Delete website (no jobs to cancel)
        delete_response = await test_client.delete(f"/api/v1/websites/{website_id}")
        assert delete_response.status_code == 200
    ...
    async def test_delete_website_archives_config(
        ...
        create_response = await test_client.post(
            ...
            json={
                "name": "Archive Test",
                "base_url": "https://example-archive.com",
                "steps": [{"name": "crawl", "type": "crawl", "method": "api"}],
            },
        )
        website_id = create_response.json()["id"]
